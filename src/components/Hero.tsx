...
export default function Hero({ mode }: Props) {
  ...
  // Save — guarda siempre que se pulse; muestra confirmación breve y vuelve al estado normal
  ...
  useEffect(() => { localStorage.setItem(TODAY_WATER_KEY, JSON.stringify(water)); }, [water]);
  const toggleDrop = (i: number) => setWater((w) => w.map((v, idx) => idx === i ? !v : v));

  // Energy
  const [energy, setEnergy] = useState<number>(() => loadJSON(TODAY_CHECKIN_KEY, 0));
  useEffect(() => { localStorage.setItem(TODAY_CHECKIN_KEY, JSON.stringify(energy)); }, [energy]);
